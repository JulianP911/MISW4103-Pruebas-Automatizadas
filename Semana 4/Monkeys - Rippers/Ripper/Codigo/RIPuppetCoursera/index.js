...
async function scrapLinks(page) {
  console.log("entro a scrapLinks");
...
async function recursiveExploration(page, link, depth, parentState) {
  ...
  await page.goto(link, { waitUntil: "domcontentloaded" }).catch((err) => {
    console.log(err);
    return;
  });
...
async function stateAlreadyVisited(html) {
  //console.log('stateAlreadyVisited: '+html);
  let path;
  let data;
...
async function getTextInputs(page, elementList) {
  await page.evaluate(() => {
    return new Promise((resolve) => {
      setTimeout(resolve, 2000); // This will wait for 2000 milliseconds, or 2 seconds
    });
  });
  ...
  if (object.type === "input") {
    ...
    if (
      ...
      try {
        await page.evaluate(() => {
          return new Promise((resolve) => {
            setTimeout(resolve, 4000); // This will wait for 2000 milliseconds, or 2 seconds
          });
        });
      } catch (error){
        console.log("Error page evaluate: "+error);
      }
      let html = await getDOM(page);
      if (!!html) {
        let parsedHtml = parser.parse(html);
...
async function fillInput(elementHandle, page) {
  ...
  if (type === "text") {
    ...
    elementHandle.click();
    page.keyboard.type("pepegrillo");
  } else if (type === "email") {
    elementHandle.click();
    page.keyboard.type("pepe@grillo.com");
  } else if (type === "tel") {
    elementHandle.click();
